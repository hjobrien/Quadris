...
public class Cerulean {
	 ...
	 * 
	 * @return an array of moves needed to get piece to the optimal location,
	 *         should be some form of left/right, rotate, drop
	 */
	...
	/**
	 * gives AI data for what block is active and the board state it is to be
	 * placed in
	 * 
	 * @param nextBlockType
	 *            the kind of block to be analyzed
	 * @param boardState
	 *            the current board state
	 * @throws BoardFullException
	 *             if the AI cannot place a block without over-filling the board
	 */
	 ...
	 * 
	 * @param nextBlock
	 *            the block just introduced to the board
	 * @param boardState
	 *            the board state without the block entered, all tiles are not
	 *            active
	 * @return an array of moves that positions the piece in to the optimal
	 *         location
	 * @throws BoardFullException
	 *             if the board placement algorithm would have to over-fill the
	 *             board to add a new block
	 */
	private Move[] computeBestPath(Block nextBlock, Tile[][] boardState) throws BoardFullException {
		...
		for (int moveCount = 0; moveCount < 10; moveCount++) {
			...
			for (int rotCount = 0; rotCount < nextBlockCopy.getNumRotations(); rotCount++) {
				for (int slideCount = 0; slideCount < 3; slideCount++) { // from
																			// slide
																			// one
																			// to
																			// left,
	 ...
	 * 
	 * @param moveCount
	 *            starting from the far left, how many times the block must be
	 *            moved to the right
	 * @param rotCount
	 *            starting from the default configuration, the number of
	 *            rotations to the right are required
	 * @return an array of moves that first shifts the block to the left, then
	 *         some amount to the right, rotates, and issues a 'drop' command to
	 *         terminate the sequence x
	 */
	...
	/**
	 * positions a block in a copy of the board based on translation and
	 * rotation parameters
	 * 
	 * @param nextBlock
	 *            the active block
	 * @param boardState
	 *            the state of non active tiles
	 * @param moveCount
	 *            the number of left/right moves in the test arrangement
	 * @param rotCount
	 *            the number of rotations in the test arrangement
	 * @return the board with the block moved to a certain position
	 * @throws BoardFullException
	 *             if by adding a block the board would become over-filled, it
	 *             doesnt know how to move in this situation
	 */
	private Tile[][] positionBlock(Block nextBlock, Tile[][] boardState, int moveCount, int rotCount, int slideCount)
			throws BoardFullException {
	...
	/**
	 * evaluates the relative value of the board "fitness function" for machine
	 * learning
	 * 
	 * @param boardCopy
	 ...
	 * 
	 * @param colCopy
	 *            the column of tiles to analyze
	 ...
	 * 
	 * @param boardCopy
	 *            the board to be analyzed. It should not have been processed to
	 *            remove lines
	...
	/**
	 * analyzes a column of tiles for the number of voids it contains. A void is
	 * defined as any blank space with a filled, non active tile at some point
	 * above it
	 * 
	 * @param tiles
	 *            the column of tiles to be analyzed
	...
	/**
	 * analyzes each column of the board to get their height score, only scores
	 * columns under the maximum height
	 * 
	 * @param boardCopy
	 *            the board to be analyzed
	 * @return the sum of the score of all lines under the maximum one
	 *         multiplied by 0.1
	 */
	 ...
	 * 
	 * @param newWeights
	 *            the new weights to be used in the fitness function
	 */
	...
	/**
	 * the part of the code that makes the AI an AI, this method takes in
	 * certain 'species' (the weights) and their associated fitness. It then
	 * makes a new generation of species that share some behavior with their
	 * parents A child is created by first 'crossing over' the two parent genes
	 * at random points such that (parent 1) xxxxxxxx => (child) oxxooxxx
	 * (parent 2) oooooooo then each gene is determined if it should be mutated
	 * (related to the mutationFactor) and then if it is to be mutated, the
	 * value at that gene is multiplied by some value v such that 0.5 <= v <=
	 * 1.5 the final list of children includes the parents so the AI never
	 * regresses (elitist selection)
	 * 
	 * @param species
	 *            the array of all possible parents
	 * @param speciesAvgScore
	 *            the related fitness of each of the possible parents
	 * @param mutationFactor
	 *            the probability a gene at a particular locus will mutate,
		...
		for (int i = 2; i < numSpecies; i++) {
			...
			for (int j = 0; j < child.length; j++) {
				...
				if (doMutate < mutationFactor) {
					double mutationAmount = rand.nextDouble() + 0.5; // shifts
																		// the
																		// random
																		// from
																		// 0-1
